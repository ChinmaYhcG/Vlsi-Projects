...
Verilog implementations of digital logic circuits, built as part of a self-directed VLSI learning roadmap. All designs are simulated using Icarus Verilog and GTKWave (UART also verified in Vivado) on a Linux (WSL2) environment.
...
## 3. FSM — Mealy vs. Moore Comparison

Same read-cycle controller (`go`, `ws` in; `rd`, `ds` out — a `go`-triggered read with an optional wait-state extension) implemented twice, once as a Mealy machine and once as a Moore machine, to compare output timing and state count for identical behavior.

| File              | Style | Notes                                                  |
| ----------------- | ----- | ------------------------------------------------------- |
| `mealy_fsm.v`      | Mealy | 3 states (IDLE/READ/DLY); outputs combinational on state + inputs |
| `moore_fsm.v`      | Moore | 4 states (adds DONE); outputs depend only on state       |

Each has its own self-checking testbench (`mealy_fsm_tb.v`, `moore_fsm_tb.v`).

**Simulate:**
```bash
cd fsm
iverilog -o mealy_sim mealy_fsm.v mealy_fsm_tb.v
vvp mealy_sim

iverilog -o moore_sim moore_fsm.v moore_fsm_tb.v
vvp moore_sim
```

## 4. UART Transmitter/Receiver Controller

Full UART TX/RX implementation — shared baud-rate generator, Cummings three-always-block FSM style, 16x-oversampled receiver with 2-flop metastability synchronization, self-checking loopback verification, `frame_err` framing-error detection, and back-to-back frame stress testing. Synthesizes cleanly in Vivado 2025.2 targeting a Kintex-7.

See [`uart/README.md`](./uart/README.md) for full module breakdown, design notes, and simulate/synthesize instructions.

## Tools Used

- Icarus Verilog — simulation and compilation
- GTKWave — waveform viewing
- Xilinx Vivado — simulation and synthesis (UART)
